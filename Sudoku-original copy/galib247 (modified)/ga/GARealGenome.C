...
//GA1DArrayAlleleGenome(unsigned int length, const GAAlleleSet<float> & s,
                      GA1DArrayAlleleGenome(unsigned int in_length, const GAAlleleSet<float> & s,
		      GAGenome::Evaluator f, void * u) :
//GA1DArrayGenome<float>(length, f, u){
    GA1DArrayGenome<float>(in_length, f, u){
  naset = 1;
...
GARealBlendCrossover(const GAGenome& p1, const GAGenome& p2,
  ...
  if(c1 && c2){
	...
	dist = dad.gene(i) - mom.gene(i);
      float lo = (GAMin(mom.gene(i), dad.gene(i))) - 0.5F*dist;
      float hi = (GAMax(mom.gene(i), dad.gene(i))) + 0.5F*dist;
