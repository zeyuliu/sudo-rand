...
float gaDefLinearScalingMultiplier   = 1.2F;
float gaDefSigmaTruncationMultiplier = 2.0F;
float gaDefPowerScalingFactor        = 1.0005F;
float gaDefSharingCutoff             = 1.0F;
...
GASharing::evaluate(const GAPopulation& p) {
  ...
  if(df) {
    for(i=0; i<n; i++){		// calculate and cache the distances
      d[i*n+i] = 0.0F;		// each genome is same as itself
      for(j=i+1; j<n; j++)
